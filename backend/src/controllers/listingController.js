...
const getListings = async (req, res) => {
  try {
    const { location, category, minPrice, maxPrice, sort, search, country } = req.query;
    let query = {};
...
const getFeaturedListings = async (req, res) => {
  try {
    ...
    if (country) query.country = { $regex: country, $options: 'i' };
    const listings = await Listing.find(query).limit(8).populate('host', 'name email avatar');
    res.json(listings);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
const createListing = async (req, res) => {
  try {
    const { title, description, price, location, country, category, amenities, maxGuests, bedrooms, bathrooms, images, isFeatured } = req.body;
